 ...
 */
function translitePinyin(navArr, sideBar) {
  navArr.forEach(nav => {
    if (nav.link) {
  ...
  Object.keys(sideBar).forEach(url => {
    let urlPinyin = slugify(url, {ignore: ['/', '.']})

    sideBar[urlPinyin] = sideBar[url]
    delete sideBar[url]
  })
}
